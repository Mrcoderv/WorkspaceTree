...
function activate(context) {
    ...
    const exportCommand = vscode.commands.registerCommand("workspace-tree.export", handleExportCommand);
...
async function handleExportCommand() {
    ...
    if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showWarningMessage("WorkspaceTree: No workspace folder is open. Please open a folder first.");
        return;
    }
    ...
    try {
        ...
        // 6. Success feedback
        vscode.window.showInformationMessage(`WorkspaceTree: Copied to clipboard! ` +
            `(${result.dirCount} folders, ${result.fileCount} files)`);
        log(`Export complete. Dirs: ${result.dirCount}, Files: ${result.fileCount}`);
    }
    catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        vscode.window.showErrorMessage(`WorkspaceTree: Export failed — ${message}`);
        log(`ERROR: ${message}`);
    }
    finally {
...
async function promptForOptions() {
    ...
    return {
        includeHidden: hiddenPick.value,
        pathFormat: pathPick.value,
        exportType: formatPick.value,
    };
}
// ---------------------------------------------------------------------------
// Tree generation
 ...
 */
async function generateTree(rootPath, options) {
    ...
    const rootNode = await buildTreeNode(rootPath, options, excludePatterns, 0 // depth counter (for safety cap)
    );
    ...
    if (options.exportType === "markdown") {
        ...
        lines.push("```");
        lines.push("");
        lines.push(`> Generated by **WorkspaceTree** — ${fileCount} files, ${dirCount} folders`);
    }
    else {
        lines.push(`${rootName}/`);
 ...
 */
async function buildTreeNode(nodePath, options, excludePatterns, depth) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            if (depth < MAX_DEPTH) {
                const childNode = await buildTreeNode(childPath, options, excludePatterns, depth + 1);
 ...
 */
function shouldExclude(name, options, excludePatterns) {
 ...
 * ```
 */
function renderMarkdown(nodes, lines, indent, options, rootPath, counters) {
    nodes.forEach((node, index) => {
        ...
        if (node.isDirectory) {
            ...
            renderMarkdown(node.children, lines, childIndent, options, rootPath, counters);
        }
        else {
            counters.file(node);
            lines.push(`${indent}${connector}📄 ${displayPath}`);
 ...
 * ```
 */
function renderPlainText(nodes, lines, indent, options, rootPath, counters) {
    nodes.forEach((node) => {
        const displayPath = formatNodePath(node, options);
        if (node.isDirectory) {
            counters.dir(node);
            lines.push(`${indent}${displayPath}/`);
            renderPlainText(node.children, lines, indent + "  ", options, rootPath, counters);
        }
        else {
            counters.file(node);
            lines.push(`${indent}${displayPath}`);
 ...
 */
function formatNodePath(node, options) {
